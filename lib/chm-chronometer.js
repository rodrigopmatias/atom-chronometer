export default class Chronometer {
  ...
  get currencySymnbol() {
    return this._currencySymnbol;
  }

  set currencySymnbol(value) {
    this._currencySymnbol = value;
  }
  ...
  set fractionDigits(value) {
    this._fractionDigits = Number.parseInt(value, 10);
  ...
  value() {
    const fill = (n, size) => Number.parseInt(n || 0, 10).toLocaleString(
      'en-us',
      {
    ...
    const timerValue = [fill(this.hour(), 2), fill(this.minute(), 2), fill(this.second(), 2)].join(':');
    ...
    return `${timerValue} - ${this.currencySymnbol} ${formatedValue}`;
  }
  ...
  init(locale, hourValue, currencySymnbol, fractionDigits) {
    this.locale = (locale || 'en-us');
    this.currencySymnbol = currencySymnbol || 'USD';
